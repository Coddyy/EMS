...
class Candidate extends MY_Controller  
{
  ...
  function __construct()
   {
     parent::__construct();
		...
		$this->load->library('form_validation');
     	...
     	if($this->uri->segment('2')=='email_verify' || $this->uri->segment('3')=='search' || $this->uri->segment('3')=='index' || $this->uri->segment(3)=='popup' || $this->uri->segment(2)=='change_email' || $this->uri->segment(2)=='email_changed_successfully'  || $this->uri->segment(2)=='quick_apply'|| $this->uri->segment(2)=='quick_apply_flashdata' || $this->uri->segment(2)=='cvupload' || $this->uri->segment(2)=='already_applied' || $this->uri->segment(2)=='quick_apply_success' || $this->uri->segment(2)=='more_skill_details_test' || $this->uri->segment(2)=='user_details' || $this->uri->segment(2)=='instructions' || $this->uri->segment(2)=='give_exam' || $this->uri->segment(3)=='success' || $this->uri->segment(3)=='exam' || $this->uri->segment(2)=='save_exam' || $this->uri->segment(2)=='googleLogin')
     	{
     		//Do nothing
     		//echo 'Here';exit();
     	}
		...
		if(isset($_POST['login']))
		{
			...
			if($this->form_validation->run() == TRUE)
			{
				...
				if($result != false)
				{
					  ...
					  if($return_url != NULL && $job_id != NULL && $return_url !='exam')
					  {
					  	//echo "Q1".$job_id;exit();
					  	$candidate_id=$this->session->userdata('candidate_id');
						...
						if($this->uri->segment(3) == "exam")
					  	{
		            		...
		            		else
		            		{
								...
								$uapply_id=$this->db->insert_id();

								$apply_id=md5($uapply_id).'c1s8'.$uapply_id.'c1s8'.md5($uapply_id + 1);

								//echo 'a1'.$apply_id;exit();
	...
	//	print_r($this->session->all_userdata());
		if(isset($_POST['savePassMobInformation']))
		{
			//var_dump($_POST);exit();
        ...
        $this->db->order_by('city_name','ASC');
      	...
      	$passresult=$this->db->get('candidate');
      	//echo $locresult->row()->location;exit();
      	$this->data['candPassword']=$passresult->row()->password;

      	$this->db->where('id',$candidate_id);
      	$this->db->select('location');
      	$locresult=$this->db->get('candidate');
      	//echo $locresult->row()->location;exit();
      	$this->data['candLocation']=$locresult->row()->location;
      	if($locresult->row()->location)
      	{
      		$loc=$locresult->row()->location;
      		$this->db->order_by('created','DESC');
      		$query=$this->db->get_where('job', array('location'=>$loc));
      		// echo '<pre>';
      		// print_r($query->result());
      		$this->data['nearbyjobs']=$query->result();
      	}
   ...
   public function phone_exists()
	...
	public function quick_apply_flashdata()
	{
		if($_POST)
		{
			$t=$this->input->post('t');
			$c=$this->input->post('c');
			$this->Candidate_M->set_qt($t,$c);
		}
		$this->load->view('candidate/subview/quick');
	}
	...
	public function give_exam()
	{
		...
		if($this->Candidate_M->is_exam_start_time_set($apply_id) == TRUE)
		{
			// Do Nothing 
		}
		else
		{
			$uedata['exam_start_time']=date('Y-m-d H:i:s');
			$this->db->where('apply_id',$apply_id);
			$this->db->update('applied_exams',$uedata);
		}
		...
		if($_POST)
		{
			if($this->Candidate_M->check_is_there_exam_id($exam_id,$candidate_id,$apply_id) != false)
			{
				...
				if($success)
				{
					...
					$uapply_id=md5($oapply_id).'c1s8'.$oapply_id.'c1s8'.md5($oapply_id + 1);

					$endtime['exam_end_time']=date('Y-m-d H:i:s');
					$this->db->where('apply_id',$oapply_id);
					$this->db->update('applied_exams',$endtime);

					redirect(base_url()."candidate/give_exam/".$oexam_id."/".$ocandidate_id."/".$uapply_id,'refresh');
				}
			}
			else
			{
	...
	public function instructions()
	{
		...
		// $ex_apply_id=explode('c1s8', $apply_id);
		// $right_side=explode('c1s8',$ex_apply_id[1]);
		// $dapply_id=$right_side[0];
	...
	public function my_exams($apply_id = NULL)
	{
		if($apply_id)
		{
			$udata['is_over']='YES';
	...
	{
		$data['subview']='candidate/subview/request_certificate';
		$this->load->view('_layout_home',$data);
	}
	public function requested($url_apply_id)
	{
		$ex_apply_id=explode('1a8c', $url_apply_id);
		$right_side=explode('1a8c',$ex_apply_id[1]);
		$apply_id=$right_side[0];

		$rand='';
		$seed1 = str_split('ABCDEFGHIJKLMNOPQRSTUVWXYZ' .'0123456789');
		shuffle($seed1); 
		foreach (array_rand($seed1, 3) as $k) 
		$rand1.= $seed1[$k];

		$seed2 = str_split('ABCDEFGHIJKLMNOPQRSTUVWXYZ'.'0123456789');
		shuffle($seed2); 
		foreach (array_rand($seed2, 3) as $k) 
		$rand2.= $seed2[$k];
	
		$cadcat_id=$rand1.$apply_id.$rand2;
		//echo $cadcat_id;exit();

		$data['apply_id']=$apply_id;
		$data['CADcat_id']=$cadcat_id;
		$data['created']=date('Y-m-d H:i:s');

		$this->db->insert('certificate',$data);

		$udata['certificate_status']=1;
		$this->db->where('apply_id',$apply_id);
		$this->db->update('certificate',$udata);

		$this->session->set_flashdata('requested','Your request has been sent successfully. Redirecting... ');
		redirect(base_url()."candidate/request_certificate/".$url_apply_id);
 ...
 //    }
	...
	public function googleLogin()
	{
		...
		if($query->num_rows() > 0)
		{
			//echo json_encode('EmailAlreadyRegistered');
