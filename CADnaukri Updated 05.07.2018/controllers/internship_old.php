  ...
  function __construct()
   {
     parent::__construct();
		...
		$this->load->library('form_validation');
     	...
     	if($this->uri->segment(2)=='email_verify' || $this->uri->segment(2)=='internship_details' || $this->uri->segment(2)=='quick_apply' || $this->uri->segment(2)=='quick_apply_success' || $this->uri->segment(2)=='sign_up_success' || $this->uri->segment(2)=='registration_successfully' || $this->uri->segment(2)=='set_password' || $this->uri->segment(2)=='internships')
     	{
     		//
     	}
   ...
   //          $message .= "</body></html>";
		...
		else if($site == 'personal_details')
    	{
    		if($_POST)
			{
				...
				if(isset($_FILES['image']['name']))
				{
					...
					// echo $_POST['name1'];
					$config['upload_path']='./intern_image/';
						...
						else
						{
						    ...
						    $iname=$_FILES["image"]["name"];
						   	$data['image_path']='intern_image/'.rand(153,999999999).$iname;
						   	echo $data['image_path'];exit();
						}
	...
	{	
		...
		if($this->form_validation->run() == TRUE)
		{
			...
			if($this->Intern_M->check_email_exist()==TRUE)
			{
				...
				$this->session->set_flashdata('sucess','Check your Email Box');
				redirect(base_url('internship/login'),'refresh');
			}
			else
			{
	...
	public function quick_apply()
	{
		...
		//echo urlencode("https://www.softcadinfotech.in/cv/testcv12.pdf");
		//echo urldecode("https%3A%2F%2Fwww.softcadinfotech.in%2Fcv%2Ftestcv12.pdf");
		$data['subview']='internship/subview/quick_apply';
		$this->load->View('_layout_home',$data);
	}
	...
	{
		$data['subview']='internship/subview/update_passwordc';
		$this->load->view('_layout_home',$data);
	}
}