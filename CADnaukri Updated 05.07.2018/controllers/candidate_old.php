  ...
  function __construct()
   {
	...
	//***************************************************** END **************************************************//
				...
				else
				{
					...
					if($this->uri->segment(3) == "popup")
					{
						redirect(base_url('candidate/login/popup'),'refresh');
					}
					else
					{
    ...
    public function profile($site=NULL)
    {
		...
		if($site == 'more_skill_details' )
		{
			...
			if($_POST)
			{
				...
				// for($i=0;$i<count($msd_id);++$i)
				// {
				// 	$msdata['msd_id'][$i]=$msd_id[$i];
				// 	$msdata['user_id'][$i]=$_POST['hidden_user_id'][$i];
				// 	$msdata['skill_id'][$i]=$_POST['hidden_skill_id'][$i];
				// 	$msdata['catagory'][$i]=$_POST['catagory'][$i];
				// 	$msdata['certification'][$i]=$_POST['certification'][$i];
				// 	$msdata['proficiency'][$i]=$_POST['proficiency'][$i];

				// 	$this->db->where('msd_id',$msd_id[$i]);
				// 	$this->db->update('more_skill_details',$msdata[$i]);
				// }
		...
		if($site == 'skill_details' )
		{
			...
			if($_POST)
			{
				...
				if(!empty($_POST['keyskill']))
				{
					$candidate_id=$this->session->userdata('candidate_id');
					$i=0;
					$this->db->where('user_id',$this->session->userdata('candidate_id'));
					$this->db->delete('more_skill_details');
					...
					foreach($_POST['keyskill'] as $key=>$val)
					{
						...
						$sdata[$i]['user_id']=$candidate_id;
						$sdata[$i]['skill_id']=$_POST['keyskill'][$key];

						// ************ //
						$sql="SELECT skill_id FROM more_skill_details WHERE user_id='$candidate_id';";
						$query=$this->db->query($sql);
						$skills=$query->result();
						var_dump($skills);exit();
						// ************ //

						$sdata[$i]['created']=date('Y-m-d H:i:s');
						//var_dump($sdata);exit();
						$i++;
					}
					$this->db->insert_batch('more_skill_details',$sdata);
