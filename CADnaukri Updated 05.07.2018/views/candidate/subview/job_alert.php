		...
		<div class="col-md-8 col-xs-10" style="box-shadow: 2px 2px 5px 5px  #cccccc;margin-top: -18px;">
			<form class="form-horizontal" id="job_alert" action="">
    			...
    			<div class="form-group">
      				...
      				<div class="col-sm-7">  
      					<select size="1" class="form-control" name="salary" id="salary" required>
    						  <?php 
                  ...
                  ?>
        			...
        			<div class="col-sm-2" style="margin-top: 5px;"><small style="font-size: 14px;color:red;"> Lacs per year</small></div>
    			</div>
    			<div class="form-group">
      				<label class="control-label col-sm-3" for="skill">Skill :</label>
      				<div class="col-sm-7">  
      					<select size="1 " class="form-control" name="skill" id="skill" required>
    			...
    			<div class="form-group">
      				...
      				<div class="col-sm-7">  
      				<select size="1" class="form-control" name="industry_type" id="industry_type" required>
    			...
    			<div class="form-group" style="font-size: 16px;">
      				...
      				<div class="col-sm-7"> 
				    <select size="1" class="form-control" name="email_frequency" id="email_frequency" required>
    						  <?php 
    			...
    			<style type="text/css">
				    ...
				    <div class="col-sm-12" align="center">
              			...
              			<input type="hidden" name="name" value="<?php echo $this->session->userdata('name');?>"></input>
              			<input type="hidden" name="mobile" value="<?php echo $this->session->userdata('mobile');?>"></input>
              			<input type="hidden" name="email" value="<?php echo $this->session->userdata('email');?>"></input>
            </form>
            		<?php if($this->Candidate_M->check_alert_staus($this->session->userdata('email')) == 1)
						{ ?>
							<div class="col-xs-12 col-md-4"><a href="<?php echo base_url();?>candidate/change_alert/0"><button type="button" class="btn btn-danger ">Deactivate Alert</button></a></div>

					<?php } else { ?>
				      	

				      	<div class="col-xs-12 col-md-4"><a href="<?php echo base_url();?>candidate/change_alert/1"><button type="button" class="btn btn-success ">Activate Alert</button></a></div>
...
<br>			
<input type="hidden" id="c_id" value="<?php echo $this->session->userdata('candidate_id');?>"></input>
<script type="text/javascript">
  ...
  $(document).ready(function(){
          ...
          $('#job_alert').submit(function(e){
            ...
            $.ajax({
              ...
              method:"POST",
              url:"<?php echo base_url();?>candidate/job_alert",
              data:$('#job_alert').serialize(),
              success:function(data)
              {
                $('#display').html(data);
                 $('#display').fadeOut(100000);
                //$('#job_alert').reset();
              }
            });
            e.preventDefault();
  ...
  function fetch_update()
  {
  		...
  		$.ajax({
              method:"POST",
              url:"<?php echo base_url();?>candidate/load_job_alert_details",
              data:{id:id},
              success:function(value)
              {
              	var data = value.split(",");
              	//$('#displaytwo').html(data[4]);
                $('#industry_type').val(data[0]);
                $('#location').val(data[1]);
                $('#skill').val(data[2]);
                $('#salary').val(data[3]);
                $('#email_frequency').val(data[4]);
                //$('#job_alert').reset();
              }
            });
  }
