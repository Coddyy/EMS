<div>
				<ul class="breadcrumb">
					...
					<li>
						<a href="<?php echo base_url('superadmin/sponors/home_images')?>">Sponors Home Images List</a> 
					</li>
                </ul>
</div>
<div class="row-fluid sortable">
      <div class="box span12">
		 <div class="box-header well" data-original-title>
		      ...
		      ?>
               <h2><i class="icon-edit"></i>Sponsored Advertisements List(For Home Page )</h2>
               <?php 
               $href=base_url("superadmin/sponors/add_home_images");
               if(count($home_images) > 24 )
               {
			   	  $href="";
			   }
			   ?>
							   	    ...
							   	    foreach($home_images as $hm)
							   	    {
										...
										echo '<tr>';
										   echo '<td>'.$i.'</td>';
											...
											echo '<td>'.$hm->start_date .'</td>';
											echo '<td>'.($hm->end_date==''?"---":$hm->end_date).'</td>';
											echo '<td>'.($hm->status=='Y'?"Active":"Inactive") .'</td>';
											echo '<td>'.$hm->home_link .'</td>';
										echo '<td><a href="'.base_url("superadmin/sponors/add_home_images/$hm->id").'">Edit</a></td>';
										echo '</tr>';
									}
							   }
							   ?>
