...
class Candidate_M extends MY_Model 
{
  ...
  {
      //echo $candidate_id;exit();
      $sql="SELECT apply_id FROM applied_exams WHERE candidate_id='$candidate_id' AND exam_details_id='$exam_id' AND is_over='NO';";
      $query=$this->db->query($sql);
      if($query->num_rows() > 0)
      {
         return true; //Exists 
  ...
  public function check_is_there_exam_id($exam_details_id,$candidate_id,$apply_id)
  {
      echo $exam_details_id;echo '<br />';echo $candidate_id;echo '<br />'; echo $apply_id;exit();

   $sql="SELECT apply_id,exam_details_id,candidate_id FROM applied_exams WHERE exam_details_id='$exam_details_id' AND candidate_id='$candidate_id' AND apply_id='$apply_id';";
   $query=$this->db->query($sql);
   if($query->num_rows() > 0)
   {
      //var_dump($data);exit();
...
public function icheck_is_there_exam_id($exam_details_id,$candidate_id,$apply_id)
  ...
  {
      $sql="SELECT applied_exams.created,exam_details.company_name,exam_details.hiring_for,applied_exams.is_over,applied_exams.is_spam,applied_exams.exam_details_id,applied_exams.apply_id FROM applied_exams 
  ...
  {
      $sdata = array('sapply_id'=>$apply_id,
                  'exam_going'=>TRUE);
      $this->session->set_userdata($sdata); 
      //echo $this->session->userdata('sapply_id');exit();
  }
