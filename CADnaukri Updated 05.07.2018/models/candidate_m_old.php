...
class Candidate_M extends MY_Model 
{
  ...
  {
      //echo $exam_details_id;echo $candidate_id; echo $apply_id;exit();
   $sql="SELECT apply_id FROM applied_exams WHERE exam_details_id='$exam_details_id' AND candidate_id='$candidate_id' AND apply_id='$apply_id';";
   $query=$this->db->query($sql);
   if($query->num_rows() > 0)
   {
      return true; // Ok
  ...
  {
      $sql="UPDATE applied_exams SET spam='1' WHERE apply_id='$apply_id';";
      $query=$this->db->query($sql);
  }
