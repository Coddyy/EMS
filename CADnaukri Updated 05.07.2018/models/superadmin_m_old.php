    ...
    function __construct()
  {
    parent::__construct();
     ...
     //        $this->db->order_by("created", "DESC"); 
       //  $query = $this -> db -> get();
       // return $query->result();
          $sql="SELECT * FROM employer LEFT JOIN designations ON employer.roles=designations.designation_id;";
    ...
    $this->db->order_by('candidate.name','ASC');
   ...
   public function internInfo()
    ...
    public function jobInfo()
      {
        $sql="SELECT DISTINCT  job.id
    FROM `job`,`job_skills` ,`company` 
    where `job`.id=`job_skills`.job_id 
    AND `job` .companyId=`company` .id
    AND `job_skills` .company_id=`company` .id ORDER BY job.created ASC";
    ...
    public function internshipInfo()
      {
       ...
       $this->db->order_by("internsip.created", "ASC"); 
        $query = $this->db->get();
        $result = $query->result();
    return $result;
 ...
 public function getphonenum($checkbox){ 
  ...
  public function countCandidate(){
   ...
   public function searchcandidate($skill,$exeperience)
   {
    ...
    public function get_daily_polls()
    {
        $sql="SELECT * FROM daily_poll ORDER BY created DESC ;";
        $query=$this->db->query($sql);
        return $query->result();
    }
  ...
  public function location_there()
  {
  ...
  public function skill_there()
  {
      $sql="SELECT id FROM candidate_key_skills WHERE user_id='$candidate_id';";
      $query=$this->db->query($sql);
      if($query->num_rows() > 0)
      {
          return true;
  ...
  public function job_applying()
  {
      $sql="SELECT id FROM candidate WHERE (last_applied IS NOT NULL AND last_applied != '') AND id='$candidate_id';";
      $query=$this->db->query($sql);
      if($query->num_rows() > 0)
      {
          return true;
